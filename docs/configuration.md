...
| `TRUST_PROXY`         | `false`                     | `false`, `true`, or the proxy's address — `uniquelocal`, `loopback`, or a CIDR list. A hop count is refused                                                                                            |
...
| `SESSION_TTL_SECONDS` | `15811200`                  | How long a session lasts after its last use. Six months — a signed-in request more than a day — or half the lifetime, whichever is shorter — after the cookie was last issued answers with a fresh one |
