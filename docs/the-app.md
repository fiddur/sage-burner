...
(#667). It took nine goes to get there: the page began as one card kind beside a table of
...
**A heart on every card** (#479) **and on every comment** (#693). Only comments — the lines the
app wrote are done things, and hearting "withdrew this dream" means nothing.

**It says so now** (#706), which reverses the half of #693 that said it would not: no entry in the
thread and no bump up the feed still, but whoever wrote the thing gets a bell line. A heart was the
one thing anybody could do on the feed that reached nobody, so it was news only to whoever went
back and looked. One category, `hearted`, `about: 'you'` and on by default — there is no
`hearted_any`, because nobody wants every heart on the burn and a heart is the same small thing
...
`subject_account_id`. A dream has neither — it has a facilitator, who can be handed the job by
somebody else — so the author is the thread's own `offered` line, which is the only record of whose
dream it was. A `meal` and a `role` are nobody's: a sitting is generated by the hundred and a role
is the burn's, so a heart on one tells nobody rather than picking whoever happened to act first.
