...
- **`sage-burner-api-v1`** — every API read: the roster, the schedule, who you
  ...
  the routes. The two `/api/` reads that are _not_ somebody's data — the banner and
  the changelog — are named into the shell cache instead, on the argument that what a
  sign-out takes away should be what a sign-out was about.
