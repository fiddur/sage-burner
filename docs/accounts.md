...
day, off `exp - SESSION_TTL_SECONDS`. The slack keeps the renewal off every response; a response that
...
already being on the stack. Not moved and `defaultPrevented` means the app handled the click for
itself, the desktop bell's dropdown being the one that does, and it is left alone; neither means the
browser's own navigation is already under way. Deciding at capture instead swallowed that dropdown,
