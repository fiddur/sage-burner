-- Posts (#438): three categories, a third entity type and a `posted` entry kind. Five tables
-- rebuilt for the same reason as #426's — each carries a CHECK listing a vocabulary, and SQLite
-- cannot alter one in place. `db.integration.test.ts` checks each list against `enums.ts`.
...
CREATE TABLE `__new_thread` (
	...
	CONSTRAINT "thread_entity_type_check" CHECK("entity_type" in ('session', 'attendance', 'post'))
);
--> statement-breakpoint
...
CREATE TABLE `__new_thread_entry` (
	...
	CONSTRAINT "thread_entry_kind_check" CHECK("kind" in ('comment', 'offered', 'joined', 'introduced', 'posted', 'facilitator', 'helper', 'renamed', 'scheduled', 'edited', 'withdrawn')),
