-- A meal becomes a feed card (#667), the ninth thread entity type — and the last shared list with
-- no presence on the feed at all. `meals.ts` told the cook and nobody else, so a sitting filling
-- up was invisible to everybody not looking at the plan, and invisible to the digest with it.
--
-- Three categories: `meal_taken` for the burn-wide news, and the `meal_comment` pair every card
-- kind has. `thread_entry` is not rebuilt: `facilitator`, `helper`, `edited` and `comment` are all
-- already in its vocabulary.
--
-- No backfill. A meal's card is opened by the first thing that happens to it, and a burn's plan is
-- forty sittings generated in one press — opening one card each would put forty on the feed for
...
CREATE TABLE `__new_thread` (
	...
	CONSTRAINT "thread_entity_type_check" CHECK("entity_type" in ('session', 'attendance', 'post', 'song', 'bring', 'point', 'meeting', 'role', 'meal'))
);
