-- An applicant hears the answer (#476). Until now a rejection sent nothing at all, and an
-- approval sent an invite email that rides a small private mail server and lands in spam often
-- enough that it cannot be the critical path.
--
-- All three tables are rebuilt rather than altered because all three carry a CHECK listing the
-- vocabulary, and SQLite cannot alter one in place — the shape `20260806180000_general_notifications`
-- established.
