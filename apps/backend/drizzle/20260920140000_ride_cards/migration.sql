-- A journey on the rideshare board becomes a feed card (#831), the tenth thread entity type — the
-- last shared list with nothing on the feed. `rides.ts` told nobody at all, so a lift going spare
-- was seen only by whoever opened the page, and never by the digest.
--
-- Three categories: `ride_posted` for the burn-wide news, and the `ride_comment` pair every card
-- kind has. `thread_entry` is not rebuilt: `added`, `edited` and `comment` are already in its
-- vocabulary.
--
-- Backfilled, unlike the meal plan: a ride is one deliberate posting rather than one of forty
-- generated sittings, so every journey already on a board gets the card it would have opened.
-- The uuid recipe is SQLite's, because `idSchema` is `z.uuid()` and the API reads these ids back.
...
CREATE TABLE `__new_thread` (
	...
	CONSTRAINT "thread_entity_type_check" CHECK("entity_type" in ('session', 'attendance', 'post', 'song', 'bring', 'point', 'meeting', 'role', 'meal', 'ride'))
);
...
SELECT
	...
	'ride',
	`r`.`id`,
	CASE `r`.`kind` WHEN 'offers' THEN 'Offering a lift from ' ELSE 'Looking for a lift from ' END || `r`.`from`,
	NULL
FROM `ride` `r`;
--> statement-breakpoint
...
SELECT
	...
	1,
	`r`.`account_id`,
	CASE `r`.`kind` WHEN 'offers' THEN 'is offering a lift' ELSE 'is looking for a lift' END,
	`r`.`created_at`,
	NULL
FROM `ride` `r`
JOIN `thread` `t` ON `t`.`entity_type` = 'ride' AND `t`.`entity_id` = `r`.`id`;
--> statement-breakpoint
