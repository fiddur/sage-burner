...
import { registerMealRoutes, registerMealSlotRoutes } from './routes/meals.ts'
...
export const createApp = async ({
  ...
  // Separate registration, not a separate guard: the slots are the burn's shape and
  // live under `/api/admin/`, where the prefix hook is the only thing that lets them
  // through. The member-facing meal routes above are outside it.
  registerMealSlotRoutes(app, { db, sessions, now })
